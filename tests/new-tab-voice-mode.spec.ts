...
test('@voice agent composer voice button starts overlay voice mode', async ({ page }) => {
  ...
  await page.evaluate(() => {
    (window as any).__messageCountEvents = [];
    (window as any).__voiceSendEvents = [];

    window.addEventListener('persistent-layer:message-count-change', (event: Event) => {
      const detail = (event as CustomEvent).detail || {};
      (window as any).__messageCountEvents.push({
        agentId: typeof detail.agentId === 'string' ? detail.agentId : '',
