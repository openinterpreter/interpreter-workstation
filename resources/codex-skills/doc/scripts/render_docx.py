...
def calc_dpi_via_pdf(input_path: str, max_w_px: int, max_h_px: int) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="converter_pdf_") as convert_tmp_dir:
        stem = splitext(basename(input_path))[0]
        pdf_path = convert_to_pdf(input_path, convert_tmp_dir, stem)
...
def rasterize(
    ...
    with tempfile.TemporaryDirectory(prefix="converter_render_") as convert_tmp_dir:
