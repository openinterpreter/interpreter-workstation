---
name: "PowerPoint"
description: "Create, edit, render, verify, and export PowerPoint slide decks. Use when Interpreter needs to build or modify a deck, presentation deck, slide deck, slides, PowerPoint, PPT, or visually ambitious editable .pptx file."
---

# PowerPoint

Use this skill whenever the user asks Interpreter to create, generate, edit, revise, improve, render, verify, or export a deck, presentation deck, slide deck, slides, PowerPoint, PPT, or PPTX file, unless the user explicitly requests a different slide skill or tool.

This is the PPTX workflow for visually ambitious editable decks in Interpreter.

## Core Contract

- In Interpreter, use `@oai/artifact-tool` for final deck construction when it is available through the bundled runtime. The skill's `scripts/init_pro_deck_builder_js.js` helper locates the package in the bundled primary runtime cache and creates the local builder link.
- Do not improvise workspace-local `node_modules` links by hand unless the init helper has failed and you have identified the exact installed package path. Do not spend repeated shell turns probing for it.
...
- If the artifact-tool Node environment is genuinely missing after the init helper reports every checked path, continue with `python-pptx` instead of blocking on runtime installation.
- Do not start, repair, or depend on the artifact-tool Python RPC daemon.
- If you need to read PDFs locally, use the Interpreter primary runtime Python with `pypdf` since we don't have the javascript version installed.
...
- Do not use `PptxGenJS`, `pptxgenjs_helpers`, LibreOffice rendering, or screenshot-only slides.
...
When `@oai/artifact-tool` is resolvable, start deck authoring in a local builder file with:
...
Use this path when `@oai/artifact-tool` is not immediately resolvable. It still creates editable PowerPoint decks, not screenshots.
...
title = slide.shapes.add_textbox(Inches(0.6), Inches(0.45), Inches(12.1), Inches(0.7))
...
Python deck rules:
- Use editable text boxes, tables, shapes, connectors, and inserted images. Do not flatten slides into screenshots.
- Use `python-pptx` charts only when the installed library supports the required chart type cleanly; otherwise use editable tables and clearly labeled shape-based summaries.
- Verify by opening the `.pptx` as a zip package and checking `ppt/slides/slide*.xml` exists, key text appears in slide XML, and embedded media exists when images were used.
- If the deliverable needs a PDF preview and the native converter is available, use `interpreter-app tools builtin-converter convert_file ...` after saving the deck.

## Using artifact_tool Presentation APIs

Use this quick surface when editing the JS builder after proving `@oai/artifact-tool` resolves. It is a curated subset of the full TypeScript presentation API, not the full reference.
...
- If native imagegen saves into the generated images directory under Interpreter home, copy or move only the selected final images into the reference directory. Do not leave project-bound reference assets only in the default generated-images location.
...
4. Build the editable deck:
- If `@oai/artifact-tool` is resolvable, start from `scripts/init_pro_deck_builder_js.js` or write a bespoke Node builder with the same architecture: theme constants, slide data, source notes, `addText`, `addCard`, `addTitleBlock`, `addHeader`, `addPlate`, native chart helpers wrapping `slide.charts.add(...)`, table/card helpers, icon helpers, render/export helpers, and inspect-record helpers.
- If `@oai/artifact-tool` is not resolvable, write one cohesive `python-pptx` builder instead. Use the same narrative, source, visual-system, and verification discipline, but express the deck through editable `python-pptx` slide objects.
...
To scaffold a JS builder, run this skill's `scripts/init_pro_deck_builder_js.js` helper with the deck id, output builder file, slide count, reference-image directory, and output deck directory. Use the deck-specific scratch directory for the builder file and selected reference images unless the task requires a different location.
...
Run local builder files from a workspace where `@oai/artifact-tool` is resolvable. Generate builders with `scripts/init_pro_deck_builder_js.js` so the builder directory has a local `build/node_modules/@oai/artifact-tool` package link to the default Interpreter runtime package and a `package.json` with `type: "module"`. Run the generated builder with the Node executable from Interpreter workspace dependencies, or with the platform-appropriate command printed by the init script.
