// NOTE(vendored): Ported from the openai/codex js_repl kernel (codex-rs/core/src/tools/js_repl/kernel.js
// at commit ac8c9fc49, Apache-2.0) after upstream removed the feature in commit 8a559e793.
// Renames: `codex.*` helper global -> `interpreter.*`, CODEX_* env vars -> INTERPRETER_*.
// The sibling meriyah.umd.min.cjs bundle is meriyah@7.0.0 (ISC) from npm dist/meriyah.umd.min.js.
...
const meriyahPromise = import("./meriyah.umd.min.cjs").then(
  (m) => m.default ?? m,
);
...
const internalBindingSalt = (() => {
  const raw = process.env.INTERPRETER_THREAD_ID ?? "";
...
// Soft per-exec timeout: the cell is abandoned (its async work keeps running
// on the event loop) but the kernel process and committed bindings survive.
// The host keeps a hard kill backstop for kernels whose event loop is wedged.
...
const tmpDir = process.env.INTERPRETER_JS_TMP_DIR || cwd;
const homeDir = process.env.HOME ?? null;
const nodeModuleDirEnv = process.env.INTERPRETER_JS_REPL_NODE_MODULE_DIRS ?? "";
...
function getRequireForBase(base) {
  ...
  if (!req) {
    req = createRequire(path.join(base, "__interpreter_js_repl__.cjs"));
...
function nextInternalBindingName() {
  ...
  return `__interpreter_internal_commit_${internalBindingSalt}_${internalBindingCounter++}`;
}
...
async function buildModuleSource(code) {
  ...
  helperDeclarations.push(
    ...
    `const ${markCommittedFnName} = import.meta.__interpreterInternalMarkCommittedBindings;`,
    `const ${markPreludeCompletedFnName} = import.meta.__interpreterInternalMarkPreludeCompleted;`,
    "delete import.meta.__interpreterInternalMarkCommittedBindings;",
    "delete import.meta.__interpreterInternalMarkPreludeCompleted;",
  );
...
function withCapturedConsole(ctx, fn) {
  ...
  return fn(logs).finally(() => {
    // A soft-timed-out cell settles after the next cell has already installed
    // its own capture; only the still-active capture may restore the console.
    if (ctx.console === captured) {
      ctx.console = console;
    }
  });
}
...
function encodeByteImage(bytes, mimeType, detail) {
  if (bytes.byteLength === 0) {
    throw new Error("interpreter.emitImage expected non-empty bytes");
  }
  if (typeof mimeType !== "string" || !mimeType) {
    throw new Error("interpreter.emitImage expected a non-empty mimeType");
  }
...
function parseImageDetail(detail) {
  ...
  if (typeof detail !== "string" || !detail) {
    throw new Error("interpreter.emitImage expected detail to be a non-empty string");
  }
  if (!["auto", "low", "high", "original"].includes(detail)) {
    throw new Error(
      'interpreter.emitImage expected detail to be one of "auto", "low", "high", or "original"',
    );
  }
  return detail;
}
...
function normalizeEmitImageUrl(value) {
  if (typeof value !== "string" || !value) {
    throw new Error("interpreter.emitImage expected a non-empty image_url");
  }
...
function parseDataUrlMimeType(dataUrl) {
  ...
  if (!mediaType) {
    throw new Error("interpreter.emitImage expected image data URL to include a MIME type");
  }
  return mediaType;
}
...
function assertEmitImageMimeType(mimeType) {
  ...
  if (!SUPPORTED_EMIT_IMAGE_MIME_TYPES.includes(normalized)) {
    ...
    throw new Error(
      `interpreter.emitImage only supports ${supportedTypes}`,
    );
  }
}
...
function parseContentItems(items) {
  ...
  for (const item of items) {
    if (!isPlainObject(item) || typeof item.type !== "string") {
      throw new Error("interpreter.emitImage received malformed content items");
    }
    ...
    throw new Error(
      `interpreter.emitImage does not support content item type "${item.type}"`,
    );
  }
...
function parseByteImageValue(value) {
  ...
  if (!bytes) {
    throw new Error(
      "interpreter.emitImage expected bytes to be Buffer, Uint8Array, ArrayBuffer, or ArrayBufferView",
    );
  }
...
function parseToolOutput(output) {
  ...
  throw new Error("interpreter.emitImage received an unsupported tool output shape");
}

function normalizeMcpImageData(data, mimeType) {
  if (typeof data !== "string" || !data) {
    throw new Error("interpreter.emitImage expected MCP image data");
  }
  if (/^data:/i.test(data)) {
    return data;
  }
...
function parseMcpImageDetail(meta) {
  ...
  const detail = meta["interpreter/imageDetail"];
  if (
...
function parseMcpToolResult(result) {
  ...
  if (!isPlainObject(result)) {
    throw new Error("interpreter.emitImage received an unsupported MCP result");
  }
  ...
  if (!isPlainObject(ok) || !Array.isArray(ok.content)) {
    throw new Error("interpreter.emitImage received malformed MCP content");
  }
  ...
  for (const item of ok.content) {
    ...
    throw new Error(
      `interpreter.emitImage does not support MCP content type "${item.type}"`,
    );
  }
...
function requireSingleImage(parsed) {
  if (parsed.textCount > 0) {
    throw new Error("interpreter.emitImage does not accept mixed text and image content");
  }
  if (parsed.images.length !== 1) {
    throw new Error("interpreter.emitImage expected exactly one image");
  }
...
function normalizeEmitImageValue(value) {
  ...
  if (!isPlainObject(value)) {
    throw new Error("interpreter.emitImage received an unsupported value");
  }
...
const interpreter = {
  ...
  tool(toolName, args) {
    ...
    if (typeof toolName !== "string" || !toolName) {
      return Promise.reject(new Error("interpreter.tool expects a tool name string"));
    }
...
async function handleExec(message) {
  ...
  try {
    ...
    const timeoutMs =
      Number.isFinite(message.timeout_ms) && message.timeout_ms > 0
        ? Math.floor(message.timeout_ms)
        : null;

    const evaluation = execContextStorage.run(execState, async () => {
      await withCapturedConsole(context, async (logs) => {
        const cellIdentifier = path.join(
          cwd,
          `.interpreter_js_repl_cell_${cellCounter++}.mjs`,
        );
        module = new SourceTextModule(source, {
          context,
          ...
          initializeImportMeta(meta, mod) {
            ...
            meta.__interpreterInternalMarkCommittedBindings = markCommittedBindings;
            meta.__interpreterInternalMarkPreludeCompleted = markPreludeCompleted;
          },
          importModuleDynamically(specifier, referrer) {
            return importResolved(resolveSpecifier(specifier, referrer?.identifier));
          },
    ...
    if (timeoutMs === null) {
      ...
      try {
        // Promise.race keeps observing `evaluation` after a timeout, so a late
        // rejection of the abandoned cell never becomes an unhandledRejection.
        await Promise.race([
          evaluation,
          new Promise((_, reject) => {
            softTimeoutTimer = setTimeout(() => {
              reject(
                new SoftTimeoutError(
                  `js_repl execution timed out after ${timeoutMs}ms. The kernel and previously committed top-level bindings were preserved, and the timed-out code may still finish in the background; re-check its effects before retrying, and pass a larger timeout_ms for long-running operations.`,
                ),
              );
            }, timeoutMs);
          }),
