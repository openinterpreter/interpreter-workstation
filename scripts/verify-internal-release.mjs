...
if (product.distribution?.auth?.provider !== 'supabase') {
  throw new Error('Internal release is missing the official hosted authentication provider');
}
if (!product.distribution?.hostedApiBaseUrl) {
  throw new Error('Internal release is missing the official hosted API endpoint');
}
...
const { internalVersion } = JSON.parse(readFileSync(path.join(root, 'package.json'), 'utf8'));
if (!internalVersion) throw new Error('Could not resolve internalVersion');
...
console.log(
  `[internal-release] verified official distribution ${internalVersion}: ${distributables.map((item) => path.basename(item)).join(', ')}`,
);
