...
import { Check, CirclePause, CirclePlay, Flag, Loader2, Pencil, Trash2, X } from 'lucide-react';
...
export function ThreadGoalBar({ threadId, readOnly = false }: ThreadGoalBarProps) {
  ...
  return (
    <div className="flex min-h-10 shrink-0 items-center gap-2 border-b border-[var(--oa-border)] bg-[var(--oa-bg-subtle)] px-3 py-2">
      <span
        className={`size-2 shrink-0 rounded-full ${active ? 'animate-pulse bg-emerald-500' : 'bg-[var(--oa-text-faint)]'}`}
        aria-label={`Goal ${goal.status}`}
      />
      <Flag className="size-3.5 shrink-0 text-[var(--oa-text-muted)]" />
      <p className="min-w-0 flex-1 truncate text-ui-xs text-[var(--oa-text)]" title={goal.objective}>
        {goal.objective}
      </p>
      ...
      {!readOnly ? (
        <>
          <button
            type="button"
            onClick={() => void updateGoal({ status: active ? 'paused' : 'active' })}
            disabled={saving}
