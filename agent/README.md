...
**Thread Goals**: Workstation reads and edits native OIX Goals. Goals can be created, paused, resumed, updated, and cleared from the thread surface; OIX owns persistence and continuation semantics.

**Read-only remote viewing**: `RemoteThreadViewer` renders the normal virtualized thread UI from the allowlisted public-thread protocol. It contains no composer or mutation transport and can be built as the `remote-thread` browser surface.
