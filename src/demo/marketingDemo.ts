...
const MARKETING_DEMO_RESEARCH_PAPER_URL = `${import.meta.env.BASE_URL}papers/pi0-general-robot-control.pdf`;
...
export function getMarketingDemoThumbnailAssetUrl(filePath: string): string {
  return `${import.meta.env.BASE_URL}thumbnails/${encodeURIComponent(getMarketingDemoThumbnailAssetName(filePath))}.png`;
}
...
export function getMarketingDemoFileIconAssetUrl(filePath: string): string {
  return `${import.meta.env.BASE_URL}file-icons/${encodeURIComponent(getMarketingDemoFileIconAssetName(filePath))}.png`;
}
